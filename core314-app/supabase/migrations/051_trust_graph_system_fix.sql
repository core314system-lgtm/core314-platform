...
BEGIN
  FOR v_user_record IN
    SELECT DISTINCT 
      p.id AS user_id,
      p.organization_id,
      p.role AS user_role  -- FIXED: Changed from p.user_role to p.role
...
COMMENT ON FUNCTION public.fusion_trust_scoring_engine() IS 
  'Phase 43 (FIXED): Analyzes user behavior over 30 days and calculates trust scores with graph connections';
...
DO $$
BEGIN
  RAISE NOTICE 'Phase 43 Fix Applied Successfully';
  RAISE NOTICE 'Verify with: SELECT * FROM trust_graph_dashboard LIMIT 1;';
  RAISE NOTICE 'Test engine with: SELECT * FROM fusion_trust_scoring_engine();';
END $$;
