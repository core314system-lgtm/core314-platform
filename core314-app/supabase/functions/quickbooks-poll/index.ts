  ...
  try {
    ...
    for (const integration of qbIntegrations) {
      try {
        ...
        // Get access token from vault using RPC function (same pattern as all other poll functions)
        const { data: accessToken, error: vaultError } = await supabase
          .rpc('get_decrypted_secret', { secret_id: integration.access_token_secret_id });

        if (vaultError || !accessToken) {
          console.error('[quickbooks-poll] No access token found for user:', integration.user_id, vaultError?.message);
