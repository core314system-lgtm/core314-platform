...
serve(async (req) => {
  ...
  try {
    ...
    for (const integration of zoomIntegrations) {
      try {
        ...
        if (tokenError || !decryptedToken) {
          console.error('[zoom-poll] No access token found for user:', integration.user_id, tokenError);
        ...
        recordsFetched += metrics.meetingCount;
    ...
    await supabase.from('poll_run_logs').insert({
      ...
      metadata: { total_integrations: zoomIntegrations.length, errors_count: errors.length },
    });
