...
serve(async (req) => {
  ...
  try {
    ...
    // ── Hoist failure pattern fields to outer scope for title & GPT context ──
...
pattern_detected: ${patternDetected || 'none'}
pattern_title: ${patternTitle || 'none'}
pattern_confidence: ${patternConfidence !== null ? `${patternConfidence}%` : 'N/A'}
pattern_description: ${patternDescription || 'No specific failure pattern matched the detected signal categories.'}
...
1. "title": Must begin with "Operational Event Detected" followed by the pattern title. ${patternTitle ? `Use exactly: "Operational Event Detected — ${patternTitle} — ${today}"` : `Use: "Operational Event Detected — Cross-Integration Pattern — ${today}"`}
