...
import { fetchUserExecutionMode, getBaselineSupportResponse } from "../_shared/execution_mode.ts";
...
serve(withSentry(async (req) => {
  ...
  try {
    ...
    if (authHeader) {
      ...
      if (user) {
        ...
        if (executionMode === 'baseline') {
          return new Response(JSON.stringify(getBaselineSupportResponse()), {
    ...
    const systemPrompt = `You are Core314's AI Support Assistant. Help users with:
