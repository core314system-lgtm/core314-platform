...
import { fetchUserExecutionMode, getBaselineOptimizationResponse } from "../_shared/execution_mode.ts";
...
serve(withSentry(async (req) => {
  ...
  try {
    ...
    if (executionMode === 'baseline') {
      return new Response(JSON.stringify(getBaselineOptimizationResponse()), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    // ============================================================
