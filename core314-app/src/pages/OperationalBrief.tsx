...
export function OperationalBrief() {
  ...
  const [shareEmailResult, setShareEmailResult] = useState<{ type: 'success' | 'error'; text: string } | null>(null);

  const handleShareViaEmail = async () => {
    if (!brief || !shareEmailTo.trim()) return;
    ...
    try {
      ...
      const response = await fetch(`${supabaseUrl}/functions/v1/share-brief-email`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          recipientEmail: shareEmailTo.trim(),
          briefTitle: brief.title,
          healthScore: brief.health_score,
          confidence: brief.confidence,
          signals: brief.detected_signals,
          businessImpact: brief.business_impact,
      ...
      setShareEmailResult({ type: 'success', text: `Brief sent to ${shareEmailTo.trim()}` });
      setTimeout(() => {
    ...
    <div className="p-6 space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
              ...
              {shareMenuOpen && (
                <div className="absolute right-0 top-full mt-2 w-48 bg-slate-800 border border-slate-700 rounded-lg shadow-xl z-50 py-1">
                  <button
                    ...
                    onClick={() => {
                      const text = `${brief.title}\n\nHealth Score: ${brief.health_score ?? 'N/A'}\nConfidence: ${brief.confidence}%\n\nKey Signals:\n${brief.detected_signals.map(s => `- ${s}`).join('\n')}\n\nBusiness Impact:\n${brief.business_impact}\n\nRecommended Actions:\n${brief.recommended_actions.map((a, i) => `${i + 1}. ${typeof a === 'string' ? a : JSON.stringify(a)}`).join('\n')}\n\nRisk Assessment:\n${brief.risk_assessment}\n\n— Generated by Core314`;
                  ...
                  >
                    ...
                    {shareCopied ? 'Copied!' : 'Copy as Text'}
                  </button>
                  <button
                    className="w-full px-4 py-2 text-left text-sm text-slate-300 hover:bg-slate-700 flex items-center gap-2"
                    onClick={() => {
          ...
          <Button
            variant="outline"
            onClick={() => navigate('/brief/archive')}
            className="border-slate-600 text-slate-400 hover:bg-slate-800 hover:text-white"
          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-slate-800 border border-slate-700 rounded-xl shadow-2xl w-full max-w-md mx-4 p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-white">Share Brief via Email</h3>
              <button
                onClick={() => { setShareEmailOpen(false); setShareEmailResult(null); }}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full rounded-md bg-slate-900 border border-slate-600 text-white placeholder:text-slate-500 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500"
                  disabled={shareEmailSending}
                />
              </div>
              ...
              {shareEmailResult && (
                <p className={`text-sm ${
                  shareEmailResult.type === 'success' ? 'text-green-400' : 'text-red-400'
                }`}>
                  {shareEmailResult.text}
                </p>
              )}
              ...
              <div className="flex gap-3 justify-end">
                ...
                >
                  ...
                  {shareEmailSending ? 'Sending...' : 'Send Brief'}
                </Button>
              </div>
            </div>
          </div>
