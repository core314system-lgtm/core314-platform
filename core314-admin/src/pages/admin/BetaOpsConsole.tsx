...
import { ClipboardList, RefreshCw, Eye, Save, CheckCircle, XCircle, MessageSquare, Key, Plus, BarChart3, TrendingDown, Activity, Bell, Send, UserPlus, RotateCcw, Clock, AlertTriangle, CreditCard, Timer, ExternalLink, ChevronRight } from 'lucide-react';
...
type TabType = 'beta-users' | 'feedback' | 'access-codes' | 'analytics' | 'churn-intelligence' | 'reliability' | 'alerts' | 'invitations' | 'active-testers';
...
export default function BetaOpsConsole({ defaultTab = 'beta-users' }: BetaOpsConsoleProps = {}) {
  ...
  const [newCode, setNewCode] = useState({
    ...
    // Active testers lifecycle state
    ...
    useEffect(() => {
      if (activeTab === 'beta-users') {
        ...
        fetchInvitations();
      } else if (activeTab === 'active-testers') {
        fetchLifecycles();
      }
    }, [activeTab]);
  ...
  const fetchFeedback = async () => {
    ...
    const fetchLifecycles = async () => {
      try {
        ...
        if (fallbackError) {
          console.error('Error fetching lifecycles:', fallbackError);
        ...
        if (!fallbackData || fallbackData.length === 0) {
          setLifecycles([]);
          setLifecycleStats({ activeTesters: 0, convertedTesters: 0, conversionRate: 0, betaRevenue: 0 });
          setLifecycleLoading(false);
          return;
        }

        // Get profiles for display names
        const userIds = fallbackData.map((r: Record<string, unknown>) => r.user_id as string);
        const { data: profilesData } = await supabase
          .from('profiles')
          .select('id, full_name, email')
          .in('id', userIds);

        // Get beta applications for company names
        const emails = profilesData?.map(p => p.email).filter(Boolean) || [];
        const { data: appsData } = emails.length > 0
          ? await supabase.from('beta_applications').select('email, company').in('email', emails)
          : { data: [] };

        const mapped: BetaLifecycle[] = fallbackData.map((r: Record<string, unknown>) => {
          const profile = profilesData?.find(p => p.id === r.user_id);
          const app = appsData?.find((a: Record<string, unknown>) => a.email === profile?.email);
          const firstLogin = r.first_login_at ? new Date(r.first_login_at as string) : null;
          const extDays = (r.extension_days as number) || 0;
          const totalDays = 45 + extDays;
          const daysElapsed = firstLogin
            ? Math.floor((Date.now() - firstLogin.getTime()) / (1000 * 60 * 60 * 24))
            : 0;

          return {
            lifecycle_id: r.id as string,
            user_id: r.user_id as string,
            full_name: profile?.full_name || 'Unknown',
            email: profile?.email || 'Unknown',
            company: (app?.company as string) || null,
            lifecycle_status: (r.lifecycle_status as string) || 'accepted',
            beta_accepted_at: r.beta_accepted_at as string | null,
            first_login_at: r.first_login_at as string | null,
            days_elapsed: daysElapsed,
            days_remaining: Math.max(0, totalDays - daysElapsed),
            total_days: totalDays,
            total_logins: (r.total_logins as number) || 0,
            last_activity_at: r.last_activity_at as string | null,
            day_38_email_sent_at: r.day_38_email_sent_at as string | null,
            day_45_completed_at: r.day_45_completed_at as string | null,
            first_payment_at: r.first_payment_at as string | null,
            checkout_url: r.checkout_url as string | null,
        ...
        // Calculate stats
        const active = mapped.filter(l => ['accepted', 'active', 'thanked', 'completed', 'extended'].includes(l.lifecycle_status)).length;
    ...
    const createCheckoutForUser = async (lifecycle: BetaLifecycle) => {
      try {
        ...
        const url = await getSupabaseFunctionUrl('beta-create-checkout');
        const response = await fetch(url, {
          ...
          body: JSON.stringify({ user_id: lifecycle.user_id }),
        });
        const result = await response.json();
        if (result.checkout_url) {
          toast.success('Checkout URL generated');
    ...
    const extendBetaPeriod = async (lifecycle: BetaLifecycle) => {
      try {
        const { data: { session } } = await supabase.auth.getSession();
        ...
        if (error) throw error;
        toast.success(`Extended ${lifecycle.full_name}'s beta by 15 days`);
        await fetchLifecycles();
      } catch (error) {
        console.error('Error extending beta:', error);
        toast.error('Failed to extend beta period');
      }
    };
    ...
    const runLifecycleCheck = async () => {
      try {
        ...
        const url = await getSupabaseFunctionUrl('beta-lifecycle-check');
        const response = await fetch(url, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
        ...
        const result = await response.json();
        if (result.success) {
          toast.success(`Lifecycle check complete: ${result.day38_count || 0} Day 38 emails, ${result.day45_count || 0} completions`);
          await fetchLifecycles();
        } else {
          toast.error(result.error || 'Lifecycle check failed');
        }
      } catch (error) {
    ...
    const getLifecycleStatusBadge = (status: string) => {
      const styles: Record<string, string> = {
        ...
        thanked: 'bg-purple-100 text-purple-700',
    ...
    const getActivityLevel = (logins: number, days: number): string => {
      if (days === 0) return 'New';
      const rate = logins / Math.max(days, 1);
    ...
    const getNextAction = (lifecycle: BetaLifecycle): string => {
      switch (lifecycle.lifecycle_status) {
        case 'accepted': return 'Awaiting first login';
        case 'active': return lifecycle.days_elapsed >= 35 ? 'Send thank-you soon' : 'Monitor engagement';
        case 'thanked': return 'Follow up on conversion';
        case 'completed': return 'Generate checkout link';
        case 'converting': return 'Awaiting first payment';
        case 'converted': return 'Customer (no action)';
        case 'churned': return 'Win-back outreach';
        case 'extended': return 'Monitor extended period';
        default: return '—';
      }
    };
              ...
              {bulkCategorizing ? (
                ...
                <>
                  ...
                  Categorize All Unprocessed
                    ...
                    {activeTab !== 'analytics' && activeTab !== 'churn-intelligence' && activeTab !== 'reliability' && activeTab !== 'alerts' && activeTab !== 'invitations' && activeTab !== 'active-testers' && (
                      <button
                        onClick={() => {
          ...
          >
            <div className="flex items-center gap-2">
              ...
              Analytics
                ...
                <button
                  onClick={() => setActiveTab('active-testers')}
                  className={`px-4 py-2 font-medium transition-colors border-b-2 ${
                    activeTab === 'active-testers'
                      ? 'border-blue-600 text-blue-600'
                      : 'border-transparent text-gray-600 hover:text-gray-900'
                  }`}
                >
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-sm font-medium text-gray-500">Active Testers</span>
              </div>
              <p className="text-2xl font-bold text-gray-900">{lifecycleStats.activeTesters}</p>
            </div>
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
              <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-2xl font-bold text-emerald-600">{lifecycleStats.convertedTesters}</p>
            </div>
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
              <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-2xl font-bold text-purple-600">{lifecycleStats.conversionRate}%</p>
            </div>
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
              <div className="flex items-center gap-2 mb-1">
                <CreditCard className="w-4 h-4 text-green-600" />
                <span className="text-sm font-medium text-gray-500">Beta Revenue</span>
              </div>
              <p className="text-2xl font-bold text-green-600">${lifecycleStats.betaRevenue.toLocaleString()}</p>
            </div>
          </div>

          {/* Actions Bar */}
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-500">{lifecycles.length} total lifecycle records</p>
            <div className="flex gap-2">
              <button
                onClick={runLifecycleCheck}
          ...
          {lifecycleLoading ? (
            ...
            <div className="text-center py-12 bg-gray-50 rounded-lg">
              ...
              <p className="text-gray-600 font-medium">No active beta testers yet</p>
              <p className="text-sm text-gray-500 mt-1">Lifecycle records are created when beta testers are approved</p>
            </div>
          ) : (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">CC Collected</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {lifecycles.map((lifecycle) => {
                      ...
                      const progressPct = lifecycle.total_days > 0 ? Math.min((lifecycle.days_elapsed / lifecycle.total_days) * 100, 100) : 0;
                      const progressColor = progressPct >= 85 ? 'bg-red-500' : progressPct >= 70 ? 'bg-yellow-500' : 'bg-green-500';

                      return (
                        <tr key={lifecycle.lifecycle_id} className="hover:bg-gray-50">
                          <td className="px-4 py-3 whitespace-nowrap">
                            <div className="text-sm font-medium text-gray-900">{lifecycle.full_name}</div>
                            {lifecycle.company && (
                              <div className="text-xs text-gray-500">{lifecycle.company}</div>
                            )}
                          </td>
                          <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{lifecycle.email}</td>
                          <td className="px-4 py-3 whitespace-nowrap">
                            <div className="flex items-center gap-2">
                            ...
                            <div className="w-20 bg-gray-200 rounded-full h-1.5 mt-1">
                              <div className={`h-1.5 rounded-full ${progressColor}`} style={{ width: `${progressPct}%` }} />
                            </div>
                          </td>
                          <td className="px-4 py-3 whitespace-nowrap">
                            {getLifecycleStatusBadge(lifecycle.lifecycle_status)}
                          </td>
                          <td className="px-4 py-3 whitespace-nowrap">
                            <span className={`text-sm font-medium ${getActivityColor(activityLevel)}`}>
                              {activityLevel}
                            </span>
                            <span className="text-xs text-gray-500 ml-1">({lifecycle.total_logins} logins)</span>
                          </td>
                          <td className="px-4 py-3 whitespace-nowrap">
                          ...
                          <td className="px-4 py-3 whitespace-nowrap">
                            <div className="flex items-center gap-1">
                              {!lifecycle.checkout_url && lifecycle.lifecycle_status !== 'converted' && (
                                <button
                                  onClick={() => createCheckoutForUser(lifecycle)}
                                  className="px-2 py-1 text-xs bg-blue-100 text-blue-700 rounded hover:bg-blue-200 transition-colors"
                                  title="Generate Stripe Checkout URL"
                                >
                                  <CreditCard className="w-3 h-3" />
                                </button>
                              )}
                              {lifecycle.checkout_url && (
                                <a
                                  href={lifecycle.checkout_url}
                                  target="_blank"
                              ...
                              {lifecycle.lifecycle_status !== 'converted' && lifecycle.lifecycle_status !== 'churned' && (
                                <button
                                  onClick={() => extendBetaPeriod(lifecycle)}
